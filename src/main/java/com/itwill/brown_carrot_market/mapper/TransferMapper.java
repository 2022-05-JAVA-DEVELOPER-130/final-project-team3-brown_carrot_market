...
public interface TransferMapper {
	...
	@SelectKey(keyProperty = "transfer_no" , resultType = int.class , before = true ,statement = "select TRANSFER_TRANSFER_NO_SEQ.nextval from dual")
	@Insert("insert into transfer values(TRANSFER_TRANSFER_NO_SEQ.nextval, #{payment_amount}, 0, sysdate, #{user_id}, null, null, ((select user_point from userinfo where user_id=#{user_id})+#{payment_amount}))")
	int insert_Point_Deposit(int payment_amount, String user_id);
	
	@Update("update userinfo set user_point = user_point + (select * from(select transfer_deposit from transfer where user_id=#{user_id} order by transfer_date desc) where rownum =1) where user_id = #{use_id}")
	int update_Point_By_ID(String user_id);
